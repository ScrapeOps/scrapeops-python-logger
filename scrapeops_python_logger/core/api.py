...
from scrapeops_python_logger.utils.utils import merge_dicts
from scrapeops_python_logger.normalizer.proxies import ProxyNormalizer
import requests
import time
...
class SOPSRequest(object):
    ...
    @staticmethod
    def set_sops_endpoint(endpoint):
        if endpoint is not None:
            SOPSRequest.SCRAPEOPS_ENDPOINT = endpoint

    @staticmethod
    def set_sops_api_version(version):
        if version is not None:
            SOPSRequest.SCRAPEOPS_API_VERSION = version
    
    @staticmethod
    def set_sops_api_key(api_key):
        if api_key is not None:
            SOPSRequest.API_KEY = str(api_key)
